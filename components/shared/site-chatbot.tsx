...
export function SiteChatbot({ locale }: { locale: string }) {
  ...
  function startDragging(event: PointerEvent<HTMLElement>) {
    ...
    const bounds = event.currentTarget.closest<HTMLElement>("[data-chatbot-shell]")?.getBoundingClientRect();
    if (!bounds) return;
  ...
  function drag(event: PointerEvent<HTMLElement>) {
    ...
    const panel = event.currentTarget.closest<HTMLElement>("[data-chatbot-shell]")?.getBoundingClientRect();
    if (!panel) return;
    if (Math.abs(event.movementX) > 0 || Math.abs(event.movementY) > 0) dragged.current = true;
    const left = Math.min(Math.max(12, event.clientX - dragOffset.current.x), window.innerWidth - panel.width - 12);
    const top = Math.min(Math.max(12, event.clientY - dragOffset.current.y), window.innerHeight - panel.height - 12);
  ...
  return <div data-chatbot-shell className="fixed bottom-[5.5rem] right-4 z-30 sm:right-6 md:bottom-[5.5rem]" style={position ? { left: position.left, top: position.top, right: "auto", bottom: "auto" } : undefined}>
