...
import { Download, Mail, MessageCircle, Phone, RefreshCw, Search, Trash2, UserRound } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Status } from "@/components/ui/status";
import { addLeadNote, deleteLead, exportAdminLeads, getAdminLeads, getLeadDetail, updateLead, type AdminLead, type AdminProfile, type LeadDetail, type LeadFilters } from "@/lib/actions/admin-leads";
...
type Summary = { total: number; newCount: number; contactCount: number; qualifiedCount: number; travelCount: number; unassignedCount: number };
...
export function AdminLeadsPanel({ initialLeads, initialCount, initialTotalPages, specialties, profiles, summary, isAdmin }: { initialLeads: AdminLead[]; initialCount: number; initialTotalPages: number; specialties: Specialty[]; profiles: AdminProfile[]; summary: Summary; isAdmin: boolean }) {
  ...
  async function handleNote(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!result.ok) { setMessage(result.message); return; }
      event.currentTarget.reset(); setMessage(result.message || "Nota guardada.");
    } catch { setMessage("No se ha podido guardar la nota. Inténtalo de nuevo."); return; }
  ...
  return <>
    <section className="mt-10 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      {[{ label: "Todos los leads", value: summary.total, tone: "neutral" as const }, { label: "Nuevos", value: summary.newCount, tone: "warning" as const }, { label: "Contacto", value: summary.contactCount, tone: "info" as const }, { label: "Cualificados", value: summary.qualifiedCount, tone: "info" as const }, { label: "Viajes", value: summary.travelCount, tone: "success" as const }, { label: "Sin asignar", value: summary.unassignedCount, tone: "warning" as const }].map((item) => <article key={item.label} className="rounded-[1.25rem] border border-border bg-white p-5 shadow-sm"><Status tone={item.tone}>{item.label}</Status><p className="mt-4 font-display text-4xl text-sapphire">{item.value}</p></article>)}
    </section>
    ...
    <section className="mt-8 rounded-[1.5rem] border border-border bg-white p-5 shadow-soft md:p-7">
      ...
      <div className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(360px,.95fr)]">
        ...
        <LeadDetailPanel detail={activeDetail} profiles={profiles} isAdmin={isAdmin} onUpdate={updateField} onNote={handleNote} onDelete={async () => { if (!selectedLead || !window.confirm(`¿Eliminar definitivamente el lead ${selectedLead.reference}?`)) return; const result = await deleteLead(selectedLead.id); if (!result.ok) { setMessage(result.message); return; } setMessage("Lead eliminado."); await loadLeads(filters); }} />
      </div>
    </section>
  </>;
}

function LeadDetailPanel({ detail, profiles, isAdmin, onUpdate, onNote, onDelete }: { detail: LeadDetail | null; profiles: AdminProfile[]; isAdmin: boolean; onUpdate: (input: { status?: LeadStatus; assignedTo?: string | null }) => Promise<void>; onNote: (event: FormEvent<HTMLFormElement>) => Promise<void>; onDelete: () => Promise<void> }) {
  ...
  return <aside className="min-w-0 rounded-2xl border border-border bg-surface p-5 md:p-6"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Detalle de solicitud</p><h3 className="mt-2 font-display text-3xl text-sapphire">{lead.customer_name}</h3><p className="mt-1 text-sm text-muted">{lead.reference} · {leadStatusGroups[lead.status]}</p></div><Status tone={leadStatusTones[lead.status]}>{leadStatusLabels[lead.status]}</Status></div>
    ...
    <form onSubmit={onNote} className="mt-6"><label className="text-sm font-semibold text-sapphire">Añadir nota interna<textarea name="note" required maxLength={2000} rows={3} placeholder="Registra la conversación o el contexto importante…" className="mt-2 w-full resize-y rounded-xl border border-border bg-white px-3 py-3 text-sm" /></label><Button type="submit" className="mt-3">Guardar nota</Button></form>
    {isAdmin ? <div className="mt-7 border-t border-border pt-5"><Button tone="danger" type="button" onClick={() => void onDelete()}><Trash2 size={15} aria-hidden="true" />Eliminar lead</Button><p className="mt-2 text-xs text-muted">Esta acción es permanente y quedará registrada en la auditoría.</p></div> : null}
